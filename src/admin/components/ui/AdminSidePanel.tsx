  ...
  return (
    <>
      <button
        ...
        onClick={onClose}
        disabled={busy}
        className="fixed inset-0 z-[110] bg-black/70 backdrop-blur-[2px] animate-in fade-in-0 duration-200 lg:left-[17.5rem] disabled:pointer-events-none"
      />
      <aside
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-white/[0.08] px-5 py-4 sm:px-6">
          ...
          <Button
            ...
            onClick={onClose}
            disabled={busy}
            aria-label={busy ? "Saving…" : "Close"}
            className="shrink-0 border-white/[0.08] bg-white/[0.03] text-white/60 hover:bg-white/[0.06] hover:text-white disabled:opacity-40"
          >
            {busy ? (
